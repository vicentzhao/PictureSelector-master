...
public class BaseActivity extends Activity implements RadioGroup.OnCheckedChangeListener {
    /**
     * type --> 1图片 or 2视频
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        FullyGridLayoutManager manager = new FullyGridLayoutManager(BaseActivity.this, 4, GridLayoutManager.VERTICAL, false);
        recyclerView.setLayoutManager(manager);
        adapter = new GridImageAdapter(BaseActivity.this, onAddPicClickListener);
        ...
        public void onAddPicClick(int type, int position) {
            switch (type) {
                case 0:
                    ...
                    if (theme) {
                        config.setThemeStyle(ContextCompat.getColor(BaseActivity.this, R.color.blue));
                        // 可以自定义底部 预览 完成 文字的颜色和背景色
                        if (!isCheckNumMode) {
                            // QQ 风格模式下 这里自己搭配颜色，使用蓝色可能会不好看
                            config.setPreviewColor(ContextCompat.getColor(BaseActivity.this, R.color.white));
                            config.setCompleteColor(ContextCompat.getColor(BaseActivity.this, R.color.white));
                            config.setPreviewBottomBgColor(ContextCompat.getColor(BaseActivity.this, R.color.blue));
                            config.setBottomBgColor(ContextCompat.getColor(BaseActivity.this, R.color.blue));
                        }
                    }
                    if (selectImageType) {
